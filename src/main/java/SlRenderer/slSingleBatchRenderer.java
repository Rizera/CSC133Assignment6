...
public class slSingleBatchRenderer {
    ...
    private static void initOpenGL() throws Exception {
    ...
    private static float[] getVertexArray(int win_width, int win_height) {
        ...
        float[] ret_array = {xMin, yMin, xMax, yMin, xMax, yMax, xMin, yMax};
        return ret_array;
    ...
    private static void renderObjects() {
        ...
        while (!glfwWindowShouldClose(glfw_window)) {
            ...
            // compute dt here, update camera position
            if (dt > 0) {
                my_camera.defaultLookFrom.x -= dt * alpha; //move alpha to spot maybe...
